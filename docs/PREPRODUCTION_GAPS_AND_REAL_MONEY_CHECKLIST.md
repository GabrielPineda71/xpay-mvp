...
| S12 | **Análisis de vulnerabilidades** | Sin escaneo de dependencias ni análisis de código estático formal | Sí | Security Lead / herramienta automatizada | `[~]` Fase 43: script `scripts/scan-dependencies-security.sh` cubre NuGet backend y npm frontend. Hallazgos conocidos: dependencias transitivas .NET con High (Microsoft.Extensions.Caching.Memory, System.Formats.Asn1, System.Text.Json) y Moderate (Azure.Identity, Microsoft.Identity.Client); frontend npm con High (vite/esbuild) y Moderate. No se han aplicado fixes — requiere decisión explícita. Pendiente: SAST, DAST, dependency scanning automático en CI como paso bloqueante, revisión OWASP formal, auditoría externa, política de remediación por severidad. |
