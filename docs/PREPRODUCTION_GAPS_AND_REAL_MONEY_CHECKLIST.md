...
| S12 | **Análisis de vulnerabilidades** | Sin escaneo de dependencias ni análisis de código estático formal | Sí | Security Lead / herramienta automatizada | `[~]` Fase 43: script `scripts/scan-dependencies-security.sh` creado. Fase 44: upgrades controlados aplicados — EF Core 8.0.28, JwtBearer 8.0.28 (backend); vite 6.4.3, @vitejs/plugin-react 4.7.0 (frontend). Escaneo post-upgrade confirma exit 0: sin vulnerabilidades Moderate/High/Critical en NuGet ni npm. Pendiente: SAST, DAST, dependency scanning automático como CI bloqueante, revisión OWASP formal, auditoría externa, política formal de remediación por severidad. |
