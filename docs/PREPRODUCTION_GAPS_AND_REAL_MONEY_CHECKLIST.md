...
| T8 | **Ambientes separados: QA / Preproducción / Producción** | Datos de prueba y datos reales mezclados; riesgo de corrupción | Sí — crítico | Responsable técnico | `[~]` Fase 49: arquitectura Azure QA documentada en `docs/AZURE_QA_FOUNDATION.md` — Resource Group `rg-xpay-qa` separado, Azure SQL `sqldb-xpay-qa`, App Service `xpay-api-qa`, Frontend `swa-xpay-admin-qa`; 22 variables backend QA definidas; orden de scripts DB 001–008; smoke test post-deploy; checklist de salida a demo con socios; reglas de datos ficticios. Pendiente: creación real de los recursos Azure, ambiente de preproducción separado del QA, políticas de acceso por rol, backups automáticos configurados (T4), Key Vault para secretos (T9), monitoreo real activo (T5/T6), separación formal QA/Preprod/Prod antes de dinero real. |
