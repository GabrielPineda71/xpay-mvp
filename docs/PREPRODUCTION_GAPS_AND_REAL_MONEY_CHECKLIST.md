...
| T5 | **Monitoreo de uptime** | Sin alerta de caídas; no hay SLA real posible | Sí | Responsable técnico + Ops | `[~]` Fase 35: `GET /api/diagnostics/ping` (uptime probe). Fase 47: `GET /api/diagnostics/ready` (readiness probe, sin DB); matriz de probes documentada en `docs/OBSERVABILITY_AND_ALERTING_RUNBOOK.md`. Pendiente: monitor externo configurado (Azure Monitor / UptimeRobot / similar), alertas automáticas activas, dashboard, on-call formal. |
| T6 | **Alertas automáticas** | Errores financieros no detectados en tiempo real | Sí — crítico | Responsable técnico | `[~]` Fase 47: criterios de alerta documentados (matriz de 9 categorías: caída, 5xx, login failures, 429, CI failures, ausencia de auditoría; severidad Critical/High/Medium/Low; tiempo de respuesta objetivo) en `docs/OBSERVABILITY_AND_ALERTING_RUNBOOK.md`. Runbook de respuesta a incidentes definido. Pendiente: configuración real en Azure Monitor / App Insights, canales de notificación (email/SMS/Teams/PagerDuty), dashboards activos, umbrales validados con carga real, on-call formal con rotación. |
| T7 | **Estrategia de rollback técnico documentada y probada** | Corrección de errores en producción sin plan de reversión genera daño mayor | Sí | Responsable técnico | `[~]` Fase 48: runbook formal documentado en `docs/ROLLBACK_AND_RECOVERY_RUNBOOK.md` — criterios de activación (R1–R13), matriz rollback/fix-forward (Critical/High/Medium/Low), procedimientos por componente (backend/frontend/configuración), validaciones post-rollback, plantilla de acta. Pre-deploy checklist de rollback readiness agregado al QA runbook. Principio documentado: rollback técnico no revierte operaciones financieras. Pendiente: rollback real ejecutado y probado en Azure QA, snapshot automático de App Settings, backup/restore de BD probado (T4), runbook financiero de reversión de operaciones, blue/green deployment o deployment slots, automatización, simulacro formal. |
| T8 | **Ambientes separados: QA / Preproducción / Producción** | Datos de prueba y datos reales mezclados; riesgo de corrupción | Sí — crítico | Responsable técnico | `[~]` Fase 49: arquitectura Azure QA documentada. Fase 50: backend QA real desplegado — `rg-xpay-qa` separado, Azure SQL `sqldb-xpay-qa`, App Service `xpay-api-qa` (`https://xpay-api-qa.azurewebsites.net`), 23 tablas, seed QA 008, validate-backend.sh Fases 1–47 ✅. Fase 51: frontend QA real desplegado — App Service `xpay-admin-qa` (`https://xpay-admin-qa.azurewebsites.net`), SPA routing, CORS verificado, login end-to-end ✅. Fase 52: ambiente QA visible para socios — readiness verificada, datos ficticios confirmados, guion de demo preparado (`docs/PARTNER_DEMO_SCRIPT.md`, `docs/PARTNER_DEMO_READINESS.md`). Decisión: LISTA PARA DEMO con observaciones. Backend + frontend QA separados del desarrollo local y de producción. SIGUE PROHIBIDO dinero real. Pendiente: ambiente de preproducción separado del QA, políticas de acceso por rol, backups automáticos configurados (T4), Key Vault para secretos (T9), monitoreo real activo (T5/T6), separación formal QA/Preprod/Prod antes de dinero real. |
...
| S9 | **HTTPS obligatorio** | Sin certificado SSL/TLS en el ambiente de piloto local; requerido en cualquier ambiente expuesto | Sí — crítico | Responsable técnico + Ops | `[~]` Fase 46: `UseHttpsRedirection` configurable vía `Https:EnableHttpsRedirection` (default true); HSTS configurable vía `Https:EnableHsts` (default false) con `Https:HstsMaxAgeDays` (default 30 días); HSTS bloqueado explícitamente en Development; sin preload ni includeSubDomains hasta revisión formal. Pendiente: certificado TLS productivo, habilitación de HSTS en QA/producción, evaluación de preload/includeSubDomains, Front Door/App Gateway. |
...
| S11 | **Política de sesiones** | Sin tiempo máximo de sesión, sin invalidación de sesiones antiguas al cambiar contraseña | Sí | Responsable técnico | `[~]` Fase 42: duración configurable vía `Jwt:ExpirationHours` (default 2 h), ClockSkew configurable vía `Jwt:ClockSkewSeconds` (default 60 s), `ValidateLifetime=true` y `ValidateIssuerSigningKey=true` confirmados. Pendiente: refresh tokens, logout/invalidación real, revocación de sesiones, MFA admin, política por rol, monitoreo de sesiones activas. |
| S12 | **Análisis de vulnerabilidades** | Sin escaneo de dependencias ni análisis de código estático formal | Sí | Security Lead / herramienta automatizada | `[~]` Fase 43: script `scripts/scan-dependencies-security.sh`. Fase 44: upgrades controlados — EF Core 8.0.28, JwtBearer 8.0.28, vite 6.4.3, plugin-react 4.7.0; scan exit 0. Fase 45: workflow `Dependency Security Scan` en GitHub Actions — bloquea en cada push/PR a main si hay Moderate/High/Critical en NuGet o npm. Dependency scanning automático básico cubierto. Pendiente: SAST, DAST, revisión OWASP formal, auditoría externa, política formal de remediación por severidad. |
