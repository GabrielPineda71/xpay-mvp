...
// XPAY-326 — orquesta la ejecución REAL de M3-T3 (Suspend Key): lee el
...
// invoca IPassportKeyClient.SuspendKeyAsync EXACTAMENTE UNA VEZ, y
...
public static class SuspendKeyExecutor
{
        ...
        string? automatedTestReference = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(keyId))
        {
            return new(KeyOperationOutcome.LocalBlocked, null,
                "PASSPORT_TEST_NEW_KEY_ID ausente al momento de intentar Suspend Key.");
        }
        ...
        {
            // XPAY-325/326 — sin un commit SHA válido no se puede producir
        ...
        try
        {
            ...
            var evidence = SuspendKeyEvidenceBuilder.BuildSuccess(
        ...
        catch (Exception ex) when (ex is PassportAuthenticationException or PassportTransportException or PassportProtocolException)
        {
            ...
            var evidence = SuspendKeyEvidenceBuilder.BuildPassportHttpFailure(
