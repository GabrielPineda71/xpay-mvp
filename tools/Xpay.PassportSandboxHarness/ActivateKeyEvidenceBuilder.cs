...
// XPAY-332 — construye el EvidenceRecord saneado para M3-T4 (Activate Key)
// a partir del key_id real enviado y la respuesta real recibida (o su
...
// 100% testeable offline. Mirror exacto de SuspendKeyEvidenceBuilder
// (XPAY-326) — misma política de saneamiento, sin desviaciones.
//
// Política de saneamiento (idéntica a SuspendKeyEvidenceBuilder):
//   - key_id (el REAL enviado en el path, y el `id` remoto de la
//     respuesta): SOLO fingerprint (Fingerprint.Compute) — NUNCA el valor
//     real, NUNCA en `operation` (que usa un placeholder literal
//     "{key_id}", jamás el ID real interpolado).
//   - status remoto: se conserva tal cual (no es sensible).
public static class ActivateKeyEvidenceBuilder
{
    public const string CaseId = "M3-T4";

    // Placeholder literal — NUNCA el key_id real interpolado aquí.
    public const string Operation = "PATCH /v1/keys/{key_id}/activate";
    ...
    // XPAY-332 (mismo criterio XPAY-326) — bloqueo LOCAL (target ausente,
    // commit SHA no resoluble, u otra condición ANTES de intentar HTTP):
    // nunca se finge como respuesta de Passport.
    ...
    // XPAY-332 — fallo real de Passport (HTTP no-2xx, o error de protocolo
